...
export function RelatedProduct({ params }: { params: { category: string; currentProductId: string } }) {
    const { category, currentProductId } = params;
    const [relatedProducts, setRelatedProducts] = useState<Product[]>([]);

    useEffect(() => {
        const getProducts = async () => {
            const allProducts = await products();
            // Filter related products based on category and exclude the current product by ID
